...
df_minimal <- df_minimal %>% 
  mutate(
    activity_type = case_when(
      ...
      activity_type %in% c("Ride", "GravelRide", "MountainBikeRide") ~ "Cycling",
...
# create dataframe with activities and all days of the year
df_cum <- df_minimal %>%
  mutate(
  ...
  group_by(year, activity_type, doy) %>%
  summarise(
    moving_time_hr = sum(moving_time_hr, na.rm = TRUE), # sum up activities if multiple per day
    distance_km    = sum(distance_km, na.rm = TRUE), # sum up activities if multiple per day
    .groups = "drop"
  ) %>%
  group_by(year, activity_type) %>%
  complete(doy = 1:366, fill = list(moving_time_hr = 0, distance_km = 0)) %>% # add all days per year
  arrange(doy, .by_group = TRUE) %>%
  mutate(
    time_cum = cumsum(moving_time_hr), # calculate cumulative sum
    dist_cum = cumsum(distance_km) # calculate cumulative sum
  ) %>%
  ungroup()

# create blue color gradient
col_years <- pal <- pal_gradient_n(
  brewer.pal(9, "Blues")[-c(1:3)]
)(seq(0, 1, length.out = length(unique(df_minimal$year))))

# x-axis labels position
month_breaks <- yday(seq(
  as.Date("2001-01-01"),
  as.Date("2001-12-01"),
  by = "1 month"
))
...
plotting_cum_plots <- function(df) {
  
  # Check what activity types are in the data
  activity_types <- unique(df$activity_type)
  
  # Determine y variable based on activity type
  if ("Strength" %in% activity_types) {
    y_var <- "time_cum"
    y_label <- "hours"
  } else {
    y_var <- "dist_cum"
  ...
  # Create labels dataframe for the filtered data
  df_labels <- df %>%
    group_by(activity_type, year) %>%
    filter(doy == max(doy)) %>%
    ungroup()
  
  ggplot(df, aes(x = doy, y = !!sym(y_var), group = year, color = factor(year))) +
    geom_line() +
    geom_line(
    ...
    coord_cartesian(clip = "off", xlim = c(1, 366)) +
    labs(y = y_label, color = "Year") +
    theme_layout +
    theme(
...
# Split by activity_type and apply function to each
list_plot_cum <- df_cum %>%
  split(.$activity_type) %>%
  map(plotting_cum_plots)

plots_cum_sum <- wrap_plots(list_plot_cum, nrow=length(unique(df_minimal$activity_type)))
...
ggsave(
  filename = file.path(fig_dir, "cum_sum_plots.pdf"),
  plot = plots_cum_sum,
  width = 25,
  height = 10*length(activities),
  units = "cm",
  device = cairo_pdf,
  limitsize = FALSE
)
