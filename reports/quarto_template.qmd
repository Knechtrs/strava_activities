...
wrapping_plots <- function(plots_180, plots_720) {
  
  activities <- names(plots_720$plots_activity)

  # Extract all plots for each activity and flatten into a single list
  ...
  # Calculate total number of plots per activity (should be 4 in this case)
  n_plots_per_activity <- length(plots_720) + length(plots_180)
  
  # Create layout design for all activities
  # Each activity gets: AA, BB, CD (where AA and BB span full width, CD is split)
  designs <- lapply(seq_along(activities), function(i) {
    # Calculate letter offset: Activity 1 starts at A, Activity 2 at E, etc.
    offset <- (i - 1) * n_plots_per_activity  # offset: Shifts letters for each activity (0 for first, 4 for second, etc.)
    
    # Create full-width rows for the first (n_plots_per_activity - 2) plots
    # E.g., "AA", "BB" for plots 1 and 2
    full_width_rows <- sapply(1:(n_plots_per_activity - 2), function(j) {
      letter <- LETTERS[j + offset]
      paste0(letter, letter)
    })
    
    # Create split row for the last 2 plots (e.g., "CD")
    split_row <- paste0(
  ...
  # Combine all designs into a single string with newlines
  design <- paste(unlist(designs), collapse = "\n")
  
  wrap_plots(plots_to_wrap, design = design)
}
